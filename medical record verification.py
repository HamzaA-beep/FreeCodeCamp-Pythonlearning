...
def find_invalid_records(
    ...
    constraints = {
        'patient_id': isinstance(patient_id, str) and ('p' in patient_id or 'P' in patient_id),
        'age': isinstance(age, int) and age >= 18,
        'gender': isinstance(gender, str) and gender.lower() in ('male', 'female'),
        'diagnosis': isinstance(diagnosis, str) or diagnosis is None,
        'medications': isinstance(medications, list) and all([isinstance(i, str) for i in medications]),
        'last_visit_id': isinstance(last_visit_id, str) and ('v' in last_visit_id or 'V' in last_visit_id)
    }
